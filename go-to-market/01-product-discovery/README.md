# 01 — Product Discovery та позиціонування

**Статус:** done

## Мета

Зрозуміти проблему, аудиторію, альтернативи, USP і чи достатньо сильна ідея для запуску.

## Промпт

[Промпт 1 у prompts.md](../prompts.md#промпт-1)

## Артефакти цієї сесії

- [session-notes.md](session-notes.md) — уточнюючі відповіді (факти з чату)
- [output.md](output.md) — Product Summary … Elevator Pitch + рекомендації

## Що далі

Переходити до [02 Play Release](../02-play-release/). У наступні промпти підставляти:

- [../shared/product-description.md](../shared/product-description.md)
- [output.md](output.md) (аудиторія, USP, pitch)
- [../shared/decisions-log.md](../shared/decisions-log.md)
