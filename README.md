...
- Налаштування стилю відображення:
  - **Cover** / **Contain**
  - **Поворот** зображення на ±90°
  - **Позиціонування** (центр, сторони, кути)
...
- Окрема поведінка натискання для кожного віджета:
  - Відкрити конкретний віджет у налаштуваннях (за замовчуванням)
  - Відкрити додаток
  - Декоративний (без дії)
- Для віджетів без фото клік завжди відкриває саме цей віджет у додатку
- Назва віджета (кастомний підпис у списку активних віджетів у додатку)
- Для кожного віджета зберігається стабільний номер (`Віджет #N`), який не змінюється при видаленні інших
- У списку активних віджетів показується поточний розмір у ячейках (`XxY`) та **мініатюра** поточного фото
- Скидання конкретного віджета до початкового стану (очистка фото й налаштувань без видалення з робочого столу)
- Налаштування в додатку та при додаванні віджета
- Додавання віджета на робочий стіл однією кнопкою (якщо лаунчер підтримує)
